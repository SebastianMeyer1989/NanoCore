...
unless($sample_list){die "\nERROR:\nPlease specify a sample list using -s.\n\n\n\n"}
unless($reference){die "\nERROR:\nPlease specify a reference using -r.\n\n\n\n"}
unless($prefix){die "\nERROR:\nPlease specify a prefix for your run using -p.\n\n\n\n"}
unless($threads){die "\nERROR:\nPlease specify the number of threads you want to use using -t (recomended: at least 8).\n\n\n\n"}
...
open(LOG,'>',"$prefix.log");			# open a log file
print LOG "> Used parameters for this run:\n\tSample list\t\t$sample_list\n\tReference\t\t$reference\n\tPrefix\t\t\t$prefix\n\tCoverage threshold\t$threshold\n\tNumber of threads\t$threads\n";		# print parameters into log file
...
print "\n\n". `date` ."> Mapping and variant calling samples:";
print LOG "\n\n". `date` ."> Mapping and variant calling samples:";
sleep(1);
...
### Processing of Nanopore Data ###
		if($line_fields[1] eq "Nanopore" && $arraycounter==4){
			$sampleID = $line_fields[0];				# save sample ID
			$clair_nano = $line_fields[2];				# save model
			$nano_path = $line_fields[3];				# save path
			print IDS $sampleID."\n";
			...
			if(-e "Output_$prefix\_Minimap/$sampleID"){
				...
				print LOG "\n\t> Running mpileup on sample ".$sampleID."\n\n";
				system("perl $this_dir/extractPileUpFrequencies.pl --samtools_bin $samtoolsbin --BAM $bamout --outputFile $mpileupout --referenceGenome $reference");	# run mpileeup on bam file
			}
			if(-e "Output_$prefix\_Clair/$sampleID"){
				print "\tSample ".$sampleID." already called\n";
				print LOG "\tSample ".$sampleID." already called\n";
				select(undef,undef,undef, .3);
			}
			else{
				...
				print LOG "\n\t> Calling sample ".$sampleID."\n\n";
				system("run_clair3.sh -b Output_$prefix\_Minimap/$sampleID/$sampleID\_new_FLAGs.bam -f $reference -t $threads -p ont -m $clair_nano -o Output_$prefix\_Clair/$sampleID --include_all_ctgs > clair3.tmp") and die;	# run clair
...
### Processing of Illumina Data ###
		elsif($line_fields[1] eq "Illumina" && $arraycounter==5){
			$sampleID = $line_fields[0];		# save sample ID
			$clair_illu = $line_fields[2];		# save model
			$illu_path_1=$line_fields[3];		# save path
			$illu_path_2=$line_fields[4];		# save path
			print IDS $sampleID."\n";
			...
			if(-e "Output_$prefix\_Clair/$sampleID"){
				...
				print "\n\t> Calling sample ".$sampleID."\n\n";
				print LOG "\n\t> Calling sample ".$sampleID."\n\n";
				system("run_clair3.sh -b Output_$prefix\_Minimap/$sampleID/$sampleID\_new_FLAGs.bam -f $reference -t $threads -p ont -m $clair_illu -o Output_$prefix\_Clair/$sampleID --include_all_ctgs > clair3.tmp") and die;	# run clair
				system("gunzip Output_$prefix\_Clair/$sampleID/merge_output.vcf.gz") and die;	# unzip Clair output
				system("mv Output_$prefix\_Clair/$sampleID/merge_output.vcf Output_$prefix\_Clair/$sampleID/$sampleID\_merge_output.vcf") and die
			}
		}
		else{die "\nSample list line\n\n$IDandPATH\n\n has not the right format. \nFour entries needed for Nanopore data ([ID], 'Nanopore', clair model, [NANOPORE_PATH]).\nFive entries needed for Illumina data ([ID], 'Illumina', clair model, [ILLUMINA_PATH_1]), [ILLUMINA_PATH_2]).\n\n"}
	}
}
close(SAMPLELIST);
...
### Processing of Nanopore Data ###
		if($line_fields[1] eq "Nanopore"){
			...
			print "\t\tSamtools stats\n";							# create Stats summary output file
			print LOG"\t\tSamtools stats\n";
...
### Processing of Illumina Data ###
			...
			print LOG "\t".$clairout."\n";		# print sample ID to log file
			print TABLE "\n".$clairout;		# print sample ID as first column into table file
			system("cat $illu_path_1 $illu_path_2 > $prefix\_illumina_complete_tmp.fastq");	# concateneate Illumina input files
			print STATSOUTTABLE "\n".$clairout;		# print sample ID as first column into table file
			open(STATSOUT,'>',"Output_$prefix\_Stats/$clairout.coverage_per_ref");								# create Stats summary output file
			print "\t\tSamtools stats\n";
			print LOG "\t\tSamtools stats\n";
			...
			$v1= `grep -v - $prefix\_illumina_complete_tmp.fastq`;																# extract total bases
			chomp($v1);																					# chomp total bases
			print STATSOUTTABLE "\t".length($v1);														# print total bases
			...
			$v3=0;
			open(FILE, "$prefix\_illumina_complete_tmp.fastq") or die "File $prefix\_illumina_complete_tmp.fastq not found.\n\n";								# extract total reads and individual reads lengths
			while($read = <FILE>){
				chomp($read);
				$v3++;
				if($v3%4==2){
...
sleep(1);
print "\n\n". `date` ."> Starting pairwise comparison:\n";
print LOG "\n\n". `date` ."> Starting pairwise comparison:\n";
sleep(1);
